<h1>Ingrese datos Usuario Nuevo</h1>
<table align="center">
    <tr>
        <td colspan="2"><input id="rutUsuario" /></td>
    </tr>
    <tr>
        <td colspan="2"><input id="contrasenaUsuario" /></td>
    </tr>
    <tr>
        <td colspan="2">
            <select id="tipoUsuario">
                <option value="1">Usuario Normal</option>
                <option value="2">Usuario Administrador</option>
            </select>
        </td>
    </tr>
    <tr>
        <td colspan="2"><input id="nombreUsuario" /></td>
    </tr>
    <tr>
        <td colspan="2"><input id="apellidoPaternoUsuario" /></td>
    </tr>
    <tr>
        <td colspan="2"><input id="apellidoMaternoUsuario" /></td>
    </tr>
    <tr>
        <td><button id="guardarNuevoUsuario" class="boton">Guardar Nuevo Usuario</button></td><td><button id="cancelarNuevoUsuario" class="boton">Cancelar</button></td>
    </tr>
</table>
